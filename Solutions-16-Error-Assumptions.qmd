...
# Another plot of residuals versus fitted values
plot(lmod, which = 1) 
...
```{r}
n <- nobs(lmod2)
plot(tail(residuals(lmod2), n - 1) ~      # extracts (e_2, e_3, ..., e_50)
      head(residuals(lmod2), n - 1),      # extracts (e_1, e_2, ..., e_49)
...
Let $\rho$ denote the serial correlation between residuals. The hypotheses are:

- $H_0$: The residuals are uncorrelated, $\rho = 0$.
- $H_a$: The residuals are related in some way ($\rho \ne 0$, $\rho > 0$, or $\rho < 0$).
