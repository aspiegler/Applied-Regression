---
title: "Solutions Module 2: Working with Data in R"
author: 
  - name: "Adam Spiegler"
...
- [Chapter 1 R Foundations](https://jfrench.github.io/LinearRegression/r-foundations.html) from A Progressive Introduction to Linear Model
...
- `is.numeric("hello world!")` gives the output `FALSE` since it is a character and not a numeric.
- `is.numeric(1)` gives the output `TRUE` since it is a numeric (a decimal).
- `is.numeric(1L)` gives the output `TRUE` since it is a numeric (an integer).
...
::: {.callout-caution}
`x4` seems to have 3 different data types (a logical, character, and decimal). However, since vectors can only store a single data type, all are converted to characters. For example, the logical `TRUE` becomes the character `"TRUE"`.
:::
...
What would the outputs of the commands `is.list(x3)` and `is.vector(x4)` be? Without running the commands, enter your answer in the text cell below. Then check your answers by running the code cell below the solution text cell.
...
- The output of `is.list(x4)` is `FALSE` since the `c()` function creates a vector, not a list.
- The output of `is.vector(x4)` is `TRUE` since the `c()` function creates a vector. 
...
::: {.callout-caution}
When defining a vector using the combine function `c()`, numbers are assumed to be decimals unless we specify we want it to be stored as an integer by adding an `L` after the number. 

- `c(5, 6, 7, 8, 9, 10, 11)` is a vector of doubles (decimals).
- `c(5L, 6L, 7L, 8L, 9L, 10L, 11L)` is a vector of integers.
- `5:11` creates a vector of integers.
:::
...
# here's my sample
set.seed(123)
x <- rnorm(20, 100, 15) # random sample of 20 values from N(100, 15)
x
```

```{r}
# here's the output of the functions
...
range(x)  # range of x
sqrt(x)  # square root of each element in x
...
```{r}
my_mean2 <- function(x) {
	xbar <- sum(x) / length(x)  # another way to compute mean
  xbar  # print xbar to screen
}
```
...
a.  `df[2, ]` will extract only row 2 and keep all columns. 
b.  `df[, 2]` will extract all rows of data and only column 2.
c.  `df[1:2, -c(1,3)]` will extract rows one and two and exclude columns 1 and 3.
...
::: {.callout-note}
When we slice a data frame, the result is NOT always data frame. For example, slicing a single column results in a vector.
:::
...
- `df$measure >= 2.2` gives the output `FALSE`, `FALSE`, `TRUE`, `TRUE`.
- `df[df$measure >= 2.2, ]` gives the output of from rows 3 and 4 and keeps all columns.
- `df[df$measure >= 2.2, -c(1,3)]` gives the output of from rows 3 and 4 and keeps all columns except columns 1 and 3.
...
```{r}
df[df$color == "red" | df$measure > 2.2, -1]
```
