...
```{r}
class(regions_raw)  # check class of regions_mess
```
...
slice_max(hdi_2021, hdi, n = 5)
```

```{r}
# print 5 lowest hdi scores from 2021
slice_min(hdi_2021, hdi, n = 5)
```
...
Main issue is there are spaces in the column names. When there are spaces in a column name, we need to put backticks around the column name whenever referring to it in our code.
