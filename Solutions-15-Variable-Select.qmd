...
abline(1,1)  # use line C_p = p + 1 since here intercept not included in p 
```
...
- Three predictors (excluding the intercept) seems about right.
- Five predictors could be a suitable choice too.


```{r}
best_mod <- lm(Life.Exp ~ Population + Murder + HS.Grad + Frost, data = statedata)
```
