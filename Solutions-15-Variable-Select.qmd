...
Let $\mathcal{M}$ is denote the model. We have
...
step(object = nullmod, scope = formula(lmod), direction = "forward", )
```
...
```{r}
# we can start with a null or partial model
step(object = nullmod, scope = formula(lmod), direction = "both", trace = FALSE )
```
...
best_sum <- summary(best) # summarize model that minimizes RSS for each p
...
```{r}
(BIC <- best_sum$bic) # Exactly values from rs summary
...
(adjr <- best_sum$adjr) #pulls R_a^2 from regsubsets for each subset
```
